...
export async function showDownloadingLog() {
    ...
    log.log('debug', `Received request: ${JSON.stringify(request)}`);
    await wait(1000);

    // Server run the download task
    log.log('info', `[Task ${id}] Successfully parsed request.`);
    log.log(
        'debug',
    ...
    log.log('debug', `[Task ${id}] Extract URL: https://www.bilibili.com/video/BV1n2rKYcEyG/`);
    log.log('debug', `[Task ${id}] [BiliBili] BV1n2rKYcEyG: 100026+30280 with format '1080P 高清+30280 - audio only'`);

    log.log('debug', `[Task ${id}] Start download...`);
    await wait(1000);
    log.log('debug', `[Task ${id}] Finished downloading`);
    log.log('debug', `[Task ${id}] Start post processing...`);
    await wait(100);
    log.log('debug', `[Task ${id}] Finished post processing`);
    log.log(
        'debug',
        `[Task ${id}] Save video to '/home/SuniRein/Projects/yt-dlp-web/build/linux/x86_64/debug/【面对面】CR450——全球跑得最快的高铁 [BV1n2rKYcEyG].mp4'`,
    );

    log.log('info', `[Task ${id}] Download completed.`);
}
